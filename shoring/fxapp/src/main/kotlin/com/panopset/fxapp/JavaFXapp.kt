...
object JavaFXapp {
    ...
    fun launch(stage: Stage) {
        ...
        if (Stringop.isPopulated(rawFiles)) {
            ...
            for (rawPath in sortedFileSet) {
                ...
                val file = File(path)
                if (firstTime) {
                    // TODO: Do some research on this, see if the initial stage is necessary.
        ...
        Platform.runLater {
            logStage = createLogStage(fxDoc?: LogzDsiplayerCMD)
        }
    }
    ...
    private fun doSave(fxDoc: FxDoc) {
        ...
        Logz.green("Saved to " + fxDoc.fileName)
    }
    ...
    private fun createLogStage(logzDsiplayer: LogDsiplayer): Stage {
        ...
        val clearLog = createPanButton(logzDsiplayer, {
            Logz.clear()
            logTa.text = ""},
            "Clear", false, "Clear logs.")
        val refreshLog = createPanButton(logzDsiplayer, {update(logTa)},
