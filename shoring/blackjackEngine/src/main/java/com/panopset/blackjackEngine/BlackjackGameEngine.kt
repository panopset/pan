...
open class BlackjackGameEngine(val config: BlackjackConfiguration) {
    var metrics = Metrics()
    val ct = CycleController()
    ...
    fun deal(isShowing: Boolean): BlackjackCard {
        return getShoe().deal(isShowing, countingSystems)
    }
    ...
    private fun hit(handPlayer: HandPlayer) {
        handPlayer.dealCard(getShoe().deal(true, countingSystems))
    ...
    private val isShuffleNeeded: Boolean
        get() = getShoe().remaining() < getShoe().cut()
    ...
    fun getCycle(): Cycle {
        return ct.getCycle(this, getShoe(), strategy)
    }
    ...
    var automatic: Thread? = null

    private fun createAutomaticThread() {
        automatic = Thread {
            doAutomatic()
        }
        automatic!!.name = "PanAutomatic"
        automatic!!.start()
    }
        ...
        while (isAutomaticRunning() && Zombie.isActive) {
            ...
            if (CMD_DEAL == ra) {
                ...
                if (targetStake > 0 && bankroll.getStakeOutOfPlay() >= targetStake) {
                    ...
                    automatic = null //TODO got to be a better way in Kotlin land now.
                    return
                }
            }
            exec(ra)
    ...
    fun shuffle() {
        getShoe().shuffle()
        countingSystems.resetCount()
        dealerMessage = if (getShoe().isTheDeckStacked()) {
    ...
    private var shoe: BlackjackShoe? = null
    fun getShoe(): BlackjackShoe {
        if (shoe == null) {
            shoe = BlackjackShoe(config.decks)
            if (Stringop.isPopulated(config.stackedDeck)) {
                shoe!!.stackTheDeckFromDeckStacker(config)
            }
        }
        return shoe!!
    }
    ...
    fun getNewActionCycleSnapshot(action: String): CycleSnapshot {
        lastActionSnapshot = CycleSnapshot(cloneState(action))
        return lastActionSnapshot
    }

    var lastActionSnapshot = getCurrentSnapshot()
}
