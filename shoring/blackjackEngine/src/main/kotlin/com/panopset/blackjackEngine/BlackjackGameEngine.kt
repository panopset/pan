...
open class BlackjackGameEngine(val logzDsiplayer: LogDsiplayer, val config: BlackjackConfiguration) {
    constructor(config: BlackjackConfiguration) : this(LogzDsiplayerCMD, config)
    ...
    open fun exec(action: String) {
        Logz.logzDsiplayer = logzDsiplayer
        frontEndPreInitCheck()
        performAction(action)
        getNewActionCycleSnapshot(action)
    }

    private var blackjackShoe: BlackjackShoe? = null
    fun getShoe(): BlackjackShoe {
        if (blackjackShoe == null) {
            blackjackShoe = BlackjackShoe(config.decks)
        }
