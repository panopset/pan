...
class Cycle internal constructor(
    ...
    private val shoe: BlackjackShoe,
    ...
    fun deal() {
        placeBets()
        for (p in players) {
            p.activeHand?.dealCard(shoe.deal(true, countingSystems))
            bge.reportNewHand()
        }
        dealer.dealCard(shoe.deal(false, countingSystems))
        for (p in players) {
            p.activeHand?.dealCard(shoe.deal(true, countingSystems))
        }
        dealer.dealCard(shoe.deal(true, countingSystems))
        checkFor21()
        isDealt = true
        isActive = true
    }
    ...
    private fun prepareSettlement() {
        shoe.show(dealer.cards[0], countingSystems)
