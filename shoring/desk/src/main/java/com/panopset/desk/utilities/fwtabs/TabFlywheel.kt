...
class TabFlywheel: SceneUpdater() {
    ...
    private fun createBottomPane(): VBox {
        return createPanVBox(
            ...
            createPanHBox(
                createPanButton({doClear()}, "Clear", false, "Clear the list, template, and options."),
                createPanButton({doClearAll()}, "Clear all", false, "Clear the file selection, list, template, and option."),
                createPanButton({doFilter()}, "Filter", true, ""),
    ...
    private fun handleList() {
        ...
        try {
            val report = FlywheelListDriver.Builder(
                Stringop.stringToList(fwInput.text), fwTemplate.text
            )
