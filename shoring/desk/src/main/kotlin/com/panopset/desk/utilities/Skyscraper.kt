...
private fun doSend(floor: Floor) {
    ...
    when (floor.postMethodChoiceBox.value) {
        HttpMethod.GET.name -> {
            doGet(actualURL, floor.headerTextArea, floor.responseTextArea)
        }
        HttpMethod.POST.name -> {
            doPost(actualURL, floor.headerTextArea, floor.bodyTextArea, floor.responseTextArea)
        }
    }
}

private fun doPost(urlString: String, headerTextField: TextArea, bodyTextField: TextArea, responseTextArea: TextArea) {
    clrmsg(responseTextArea)
    val startTime = System.currentTimeMillis()
    processResponse(doPostHttp(createHeaders(headerTextField.text), urlString, bodyTextField.text), urlString, responseTextArea, startTime)
}

private fun doGet(urlString: String, headerTextField: TextArea, responseTextArea: TextArea) {
    clrmsg(responseTextArea)
    val startTime = System.currentTimeMillis()
    processResponse(doGetHttp(createHeaders(headerTextField.text), urlString), urlString, responseTextArea, startTime)
}
