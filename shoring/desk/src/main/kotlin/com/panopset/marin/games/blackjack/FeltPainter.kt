...
class FeltPainter() {
    ...
    @Synchronized
    fun draw(logDsiplayer: LogDsiplayer, cs: CycleSnapshot?, g: GraphicsContext, width: Int, height: Int) {
        ...
        paintPlayer(logDsiplayer, g, blackjackTable.playerTile, cs)
    ...
    private fun paintPlayer(logDsiplayer: LogDsiplayer, g: GraphicsContext, t: Tile, cs: CycleSnapshot) {
        if (dbg) {
            g.fill = Color.YELLOW
            g.fillRect(t.left.toDouble(), t.top.toDouble(), t.width.toDouble(), t.bottom.toDouble())
        }
        ...
        if (cs.getPlayers().isEmpty()) {
            logDsiplayer.dspmsg(promptDeal)
        } else {
            logDsiplayer.clear()
        }
            ...
            for (h in p.hands) {
                ...
                if (activeHand != null && h == activeHand && !h.isFinal) {
                    ...
                    if (!arrowHasBeenDrawn) {
                        g.drawImage(
                            BlackjackImages.getFxArrowImg(), (handFirstCardX + o.chipW).toDouble(),
